...
class AccountPayment(models.Model):
    ...
    def fields_get(self, allfields=None, attributes=None):
        ...
        res = super(AccountPayment, self).fields_get(allfields, attributes)
        for field in hide:
    ...
    def _prepare_move_line_default_vals(self, write_off_line_vals=None, force_balance=None):
        ...
        if self.payment_type == 'inbound':
            liquidity_amount_currency = self.amount  # Receive money
        elif self.payment_type == 'outbound':
            liquidity_amount_currency = -self.amount  # Send money
        else:
        ...
        if not self.is_cheque:
            ...
            if not self.currency_id.is_zero(write_off_amount_currency):
                line_vals_list.append({
                    'name': write_off_line_vals[0].get('name') if write_off_line_vals else '',
                    'amount_currency': write_off_amount_currency,
                    'currency_id': currency_id,
                    'debit': write_off_balance if write_off_balance > 0.0 else 0.0,
                    'credit': -write_off_balance if write_off_balance < 0.0 else 0.0,
                    'partner_id': self.partner_id.id,
                    'account_id': write_off_line_vals[0].get('account_id') if write_off_line_vals else False,
                })
        else:
            # Cheque handling
            ...
            if self.type_cheq == 'recieve_chq':
                line_vals_list = self.create_journal_lines(
                    self.journal_cheque or self.journal_id, account_id
                )
            else:
                self.name = ''
                line_vals_list = self.create_move_line_send_cheques(
                    self.journal_cheque or self.journal_id, account_id
                )
    ...
    def _get_reconsile(self, credit_account):
        ...
        if len(movee_line) > 1:
            amls = [rec.id for rec in movee_line if not rec.full_reconcile_id]
            if amls:
                movee_line = self.env['account.move.line'].search([('id', 'in', amls)])

                # Ensure partial_reconcile_ids is in the correct format (empty list if not used)
                self.env['account.full.reconcile'].create({
                    'reconciled_line_ids': [(6, 0, movee_line.ids)],
                    'partial_reconcile_ids': [(6, 0, movee_line.ids)]  # Adjust if needed
                })
