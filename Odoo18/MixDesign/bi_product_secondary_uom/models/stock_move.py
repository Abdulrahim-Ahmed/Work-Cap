...
class StockMove(models.Model):
    ...
    secondary_uom_id = fields.Many2one('uom.uom', compute='_quantity_secondary_compute', string="ALT UOM",
                                       store=True)
    secondary_quantity = fields.Float('ALT Qty', compute='_quantity_secondary_compute',
                                      digits='Product Unit of Measure', store=True)
    secondary_done_qty = fields.Float('ALT Quantity Done', compute='_quantity_secondary_done_compute',
                                      digits='Product Unit of Measure', inverse='_quantity_secondary_done_set')
    ...
    def _quantity_secondary_compute(self):
        for order in self:
            if order.product_id.secondary_uom:
                ...
                uom_quantity = order.product_id.uom_id._compute_quantity(order.product_uom_qty, order.secondary_uom_id,
                                                                         rounding_method='HALF-UP')
                order.secondary_quantity = uom_quantity

    @api.depends('quantity', 'move_line_ids.quantity', 'move_line_ids.secondary_done_qty',
            ...
            if not move_lines:
                if quantity:
                    ...
                    move_line = self.env['stock.move.line'].create(
                        dict(move._prepare_move_line_vals(), quantity=quantity))
                ...
                for move_line in move_lines:
                    ml_quantity_done += move_line.product_uom_id._compute_quantity(move_line.quantity,
                                                                                   move.secondary_uom_id, round=False)
        ...
        if res:
            ...
            if quantity:
                if self.product_id.secondary_uom:
                    uom_quantity = self.product_id.uom_id._compute_quantity(quantity, self.secondary_uom_id,
                                                                            rounding_method='HALF-UP')
                    uom_quantity_back_to_product_uom = self.secondary_uom_id._compute_quantity(uom_quantity,
...
class StockMoveLine(models.Model):
    ...
    secondary_uom_id = fields.Many2one('uom.uom', string="ALT UOM", compute="_compute_secondary_qty", store=True)
    secondary_quantity = fields.Float("ALT Qty", digits='Product Unit of Measure',
                                      compute="_compute_secondary_qty", store=True)
    secondary_done_qty = fields.Float("ALT Done Qty", digits='Product Unit of Measure')
    ...
    def _compute_secondary_qty(self):
        for move_line in self:
            if move_line.product_id.secondary_uom:
                ...
                uom_quantity = move_line.product_id.uom_id._compute_quantity(move_line.quantity or move_line.quantity,
                                                                             move_line.product_id.secondary_uom_id,
                                                                             rounding_method='HALF-UP')
                move_line.update({'secondary_quantity': uom_quantity})
                uom_done_quantity = move_line.product_id.uom_id._compute_quantity(move_line.quantity,
                                                                                  move_line.product_id.secondary_uom_id,
                                                                                  rounding_method='HALF-UP')
