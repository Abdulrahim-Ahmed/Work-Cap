...
class AccountPayment(models.Model):
    ...
    def _compute_hide_payment_method(self):
        for rec in self:
            if self.env.user.has_group("check_management_old.group_payment_method"):
    ...
    def _prepare_move_line_default_vals(self, write_off_line_vals=None, force_balance=None):
        ''' Prepare the dictionary to create the default account.move.lines for the current payment.
            ...
            :param force_balance: Optional parameter to handle force balance logic, if needed.
    ...
    def create_under_collection_journal(self):
        view = self.env.ref('check_management_old.view_account_payment_cheque_pop')
    ...
    def get_rejected_form_bank(self):
        ...
        if self.state_cheque == "payment_vendor":
            ...
            move2 = self.env['account.move'].create({'date': self.date_rejected,
                                                     'ref': "Cheque Num/" + (self.cheque_no if self.cheque_no else ''),
                                                     'partner_id': self.partner_id.id or '',
                                                     'name': self._get_payment_name(self.journal_reject,
                                                                                    self.date_rejected),
    ...
    def get_returned_to_customer(self):
        ...
        move2 = self.env['account.move'].create({'date': self.date_return,
                                                 'ref': "Cheque Num/" + (self.cheque_no if isinstance(self.cheque_no, str) else '') or '',
                                                 'partner_id': self.partner_id.id or '',
