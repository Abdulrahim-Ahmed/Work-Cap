...
class ProductTemplate(models.Model):
    ...
    secondary_uom = fields.Boolean(index=True, default=False, string='ALT uom')
    secondary_uom_id = fields.Many2one('uom.uom', 'ALT UOM')
    secondary_uom_ids = fields.Many2many('uom.uom', compute='_compute_uom_id')
    secondary_uom_name = fields.Char(string='secondary UOM', related='secondary_uom_id.name',
                                     compute='_compute_uom_name', readonly=True)
    secondary_product_qty = fields.Float(string='Qty Available', compute='_compute_secondary_quantities',
                                         search='_search_secondary_qty_available')
    ...
    def action_secondary_open_quants(self):
        return self.with_context(active_test=False).product_variant_ids.filtered(
            lambda p: p.active or p.secondary_product_qty != 0).action_secondary_open_quants()
...
class Product(models.Model):
    ...
    secondary_product_qty = fields.Float(string='Qty Available', compute='_compute_secondary_quantities',
                                         search='_search_secondary_qty_available')
    secondary_uom_id = fields.Many2one('uom.uom', string="ALT UOM", related='product_tmpl_id.secondary_uom_id',
                                       readonly=False, store=True)

    secondary_quantity = fields.Float('ALT Qty', compute='_compute_secondary_fields',
                                      digits='Product Unit of Measure')

    @api.depends('qty_available', 'secondary_uom_id')
    def _compute_secondary_fields(self):
        for product in self:
            if product.secondary_uom_id:
                # Convert primary quantity to secondary UOM
                uom_quantity = product.uom_id._compute_quantity(
                    product.qty_available,
                    product.secondary_uom_id,
                    rounding_method='HALF-UP'
                )
                product.secondary_quantity = uom_quantity
            else:
                product.secondary_quantity = 0.0
    ...
    def _search_product_quantity(self, operator, value, field):
        ...
        if field not in (
                'secondary_product_qty', 'qty_available', 'virtual_available', 'incoming_qty', 'outgoing_qty',
                'free_qty'):
    ...
    def _search_qty_available_new(self, operator, value, lot_id=False, owner_id=False, package_id=False):
        ...
        quants_groupby = self.env['stock.quant'].read_group(domain_quant,
                                                            ['product_id', 'quantity', 'secondary_quantity'],
                                                            ['product_id'])
    ...
    def _compute_quantities_dict(self, lot_id, owner_id, package_id, from_date=False, to_date=False):
        ...
        second_quants_res = {product.id: (quantity, secondary_quantity) for product, quantity, secondary_quantity in
                             Quant._read_group(domain_quant, ['product_id'],
                                               ['quantity:sum', 'secondary_quantity:sum'])}
        if dates_in_the_past:
        ...
        for product in self.with_context(prefetch_fields=False):
            ...
            if dates_in_the_past:
                ...
                second_qty_available = second_quants_res.get(origin_product_id, [0.0])[0] - moves_in_res_past.get(
