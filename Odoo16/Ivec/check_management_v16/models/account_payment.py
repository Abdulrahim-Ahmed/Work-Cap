...
INTEGRITY_HASH_LINE_FIELDS = ('debit', 'credit', 'account_id', 'partner_id', 'res.currency')
...
class AccountPayment(models.Model):
    ...
    def _compute_hide_payment_method(self):
        for rec in self:
            if self.env.user.has_group("check_management_v16.group_payment_method"):
    ...
    def _total_in_words(self):
        for rec in self:
            if rec.amount:
                rec.total_in_words = num2words(rec.amount, lang='en')
    ...
    #     }
    #     liquidity_aml_display_name_list = self._get_liquidity_aml_display_name_list()
    #     default_line_name = ''.join(
    #         x[1] for x in liquidity_aml_display_name_list) if liquidity_aml_display_name_list else ''
    #
    ...
    def _prepare_move_line_default_vals(self, write_off_line_vals=None):
        ...
        :param write_off_line_vals: Optional list of dictionaries to create a write-off account.move.line easily containing:
            * amount_currency: The amount to be added to the counterpart amount.
            * name: The label to set on the line.
            * account_id: The account on which create the write-off.
        :return: A list of python dictionary to be passed to the account.move.line's 'create' method.
        '''
        ...
        # Add write-off lines if they exist.
        for write_off_vals in write_off_line_vals:
            write_off_line = {
                'name': write_off_vals.get('name') or counterpart_line_name,
                'amount_currency': write_off_vals.get('amount_currency', 0.0),
                'currency_id': currency_id,
                'debit': write_off_vals.get('balance', 0.0) if write_off_vals.get('balance', 0.0) > 0.0 else 0.0,
                'credit': -write_off_vals.get('balance', 0.0) if write_off_vals.get('balance', 0.0) < 0.0 else 0.0,
                'partner_id': self.partner_id.id,
                'account_id': write_off_vals.get('account_id'),
            }
            line_vals_list.append(write_off_line)
    ...
    def create_under_collection_journal(self):
        view = self.env.ref('check_management_v16.view_account_payment_cheque_pop')
